...
def coin(drink_name, profit): # 동전을 투입받고 cost와 비교하여 계산하는 함수
    ...
    if cal < MENU[drink_name]['cost']: # 돈이 모자랄때 
        print("금액이 모자랍니다. 돈이 환불되었습니다.")
        return False
    elif cal >= MENU[drink_name]['cost']: # 돈이 충분할때
        print(f"\n주문하신 {drink_name}, {MENU[drink_name]['cost']:.2f}$ / 투입 금액 {cal:.2f} $")
        return cal
