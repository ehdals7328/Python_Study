...
class SmartPhone():
    ...
    def addAddr(self, person):
        ...
        print("=" * 25)
        print(f"데이터가 저장되었습니다. (현재 {len(SmartPhone.list)}개)")
        print("-" * 25)
    ...
    def deleteAddr(self):
        ...
        for obj in SmartPhone.list:
            if obj.phone == input_d:
                ...
                return print(f"{deleted_obj.name}님의 연락처가 삭제 되었습니다. ")
            print("없는 전화번호 입니다.")
            return False
    ...
    def sort_list(self):
        sorted_l = sorted(self.list, key=lambda x: x.name)
        return sorted_l
...
if __name__ == "__main__":
    while True:
        ...
        if num == '2':
            ...
            searched_obj = s_p.searchAddr()
            if not searched_obj:
                continue

        elif num == '5':
            deleted_obj = s_p.deleteAddr()
            if not deleted_obj:
                continue            

        elif num == '6':
            edited_obj = s_p.editAddr()
            if not edited_obj:
                continue
